  ...
  useEffect(() => {
    const onScrollOrResize = () => {
      ...
      if (rafRef.current == null) {
        ...
        // if RAF already scheduled, still apply step once (avoids lag)
        step()
      }
    }
...
const Section_3 = () => {
  ...
  // add small random animation delays so multiple inline tins don't bob in perfect sync
  useEffect(() => {
  ...
  // --- NEW: swap floating image src based on which section is closest to viewport center ---
  useEffect(() => {
    ...
    const chooseSrc = () => {
      ...
      // compute absolute distance from viewport center to each section's center
      ...
      // smooth crossfade: only swap if different
      if (imgEl.src && imgEl.src.endsWith(newSrc.split('/').pop())) {
        // already correct (endsWith check avoids full URL mismatch)
        return
      }

      // gentle fade-out and swap
      imgEl.style.transition = 'opacity 180ms ease, filter 180ms ease'
      imgEl.style.opacity = '0.0'
      imgEl.style.filter = 'blur(2px)'
      // after transition, swap src then fade-in
      setTimeout(() => {
        if (!floatingImgRef.current) return
  ...
  return (
    <>
      ...
      <FloatingTin startRef={section3Ref} endRef={section4RefB} imgRef={floatingImgRef} offset={0} />

      {/* -------- Section 3 (use your provided content) -------- */}
      <div ref={section3Ref}>
        {/* ... your existing Section 3 markup unchanged ... */}
        <div className='bg-[#F46C3C] lg:h-[100dvh] flex flex-col mx-auto'>
          ...
          <div className='container mx-auto flex-1 px-3 flex flex-col lg:flex-row items-center'>
            ...
            <div className='flex-1 flex flex-col w-full gap-3 order-2 lg:order-1'>
              {[
                { text: 'No Sucralose and Aspartame', color: '#F96037' },
